...
class WalletSerializer(serializers.ModelSerializer):
    class Meta:
        ...
        # balance is read-only here because it's managed by transactions and transfers
        read_only_fields = ['user', 'balance', 'created_at', 'updated_at']

class CategorySerializer(serializers.ModelSerializer):
    class Meta:
        model = Category
        # ADDED 'monthly_budget' to fields
...
class BudgetGoalSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # Budget goals can only be set for expense categories if a category is chosen
...
class RecurringBillSerializer(serializers.ModelSerializer):
    ...
    def validate(self, data):
        ...
        # Recurring bills should typically be associated with expense categories
        if 'category' in data and data['category'] and data['category'].type != 'expense':
            raise serializers.ValidationError({
